...
class GeoTIFFWriter(ImageWriter):
    """Write GeoTIFF imagery files.

    Writes single-band and multi-band arrays to GeoTIFF format with
    optional geolocation metadata (CRS and affine transform). Uses
    rasterio (GDAL) as the backend.
    ...
    metadata : ImageMetadata, optional
        Typed metadata to embed in the output file.
    ...
    >>> with GeoTIFFWriter('output.tif') as writer:
    ...     writer.write(data)
    """
    ...
    def write(
        ...
        """Write image data to a GeoTIFF file.
    ...
    def write_chip(
        ...
        """Write a spatial subset to an existing GeoTIFF file.
