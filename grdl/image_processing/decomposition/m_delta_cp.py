# -*- coding: utf-8 -*-
"""Compact-pol m-delta decomposition."""
...
class CompactPolMDelta(CompactPolDecompositionBase):
    """M-delta decomposition for compact-pol C2."""
    ...
    def component_names(self) -> Tuple[str, ...]:
        return ('surface', 'double_bounce', 'volume', 'm_cp', 'delta_cp')
    ...
    def decompose_compact(self, c11, c12, c21, c22) -> Dict[str, np.ndarray]:
        ...
        delta_rad = np.arctan2(s3, s2)

        surface = np.sqrt(np.clip((m * s0_abs * (1.0 + np.sin(delta_rad))) / 2.0, 0.0, None))
        double_bounce = np.sqrt(np.clip((m * s0_abs * (1.0 - np.sin(delta_rad))) / 2.0, 0.0, None))
        volume = np.sqrt(np.clip((s0_abs * (1.0 - m)) / 2.0, 0.0, None))
        ...
        return {
            ...
            'delta_cp': np.degrees(np.real(delta_rad)),
        }
    ...
    def to_rgb(
        ...
        return self._build_power_rgb(
            components, self._RGB_CHANNELS, 'CpMDeltaRGB',
