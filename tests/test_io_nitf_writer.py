# -*- coding: utf-8 -*-
"""
NITF Writer Tests - Unit tests for NITFWriter.
...
class TestNITFWriterSingleBand:
    ...
    def test_write_float32_roundtrip(self, tmp_path):
        ...
        from grdl.IO.nitf import NITFWriter, NITFReader

        data = np.random.rand(32, 48).astype(np.float32)
        filepath = tmp_path / "single_f32.nitf"

        with NITFWriter(filepath) as writer:
            writer.write(data)
    ...
    def test_write_float64_roundtrip(self, tmp_path):
        ...
        filepath = tmp_path / "single_f64.nitf"
    ...
    def test_write_uint8_roundtrip(self, tmp_path):
        ...
        filepath = tmp_path / "single_u8.nitf"
    ...
    def test_write_uint16_roundtrip(self, tmp_path):
        ...
        filepath = tmp_path / "single_u16.nitf"
...
class TestNITFWriterMultiBand:
    ...
    def test_write_multiband_uint8_roundtrip(self, tmp_path):
        ...
        filepath = tmp_path / "multi_u8.nitf"
    ...
    def test_write_multiband_float32_roundtrip(self, tmp_path):
        ...
        filepath = tmp_path / "multi_f32.nitf"
...
class TestNITFWriterGeolocation:
    ...
    def test_write_without_geolocation(self, tmp_path):
        """Write without geolocation produces valid file."""
        from grdl.IO.nitf import NITFWriter, NITFReader

        data = np.random.rand(16, 16).astype(np.float32)
        filepath = tmp_path / "nogeo.nitf"
...
class TestNITFWriterEdgeCases:
    ...
    def test_context_manager(self, tmp_path):
        ...
        filepath = tmp_path / "ctx.nitf"
        writer = NITFWriter(filepath)
        assert isinstance(writer, ImageWriter)
        with writer:
            writer.write(np.zeros((8, 8), dtype=np.float32))

    def test_ntf_extension(self, tmp_path):
        """Writer accepts .ntf extension."""
        from grdl.IO.nitf import NITFWriter, NITFReader
