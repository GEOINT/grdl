...
- **`gen_quicklooks`** (`grdl/discovery`): New batch SICD PNG quicklook
  generator with two multilook modes.  **Spatial-domain** (default) uses
  block-average intensity with no FFT and minimal memory.
  **Frequency-domain** (``--freq-multilook``) uses ``MultilookDecomposition``
  sub-aperture FFT with GPU acceleration and automatic CPU fallback on OOM.
  Applies Mangis density remap (``grdl.contrast.MangisDensity``) for improved
  SAR contrast matching GRDK display.  Three CUDA OOM mitigation strategies:
  GPU cache clearing between files, automatic CPU retry on OOM, and
  low-memory spatial mode.  Includes byte-order fix for big-endian SICD data
  on little-endian hosts.  CLI: ``python -m grdl.discovery.gen_quicklooks``.
  API: ``QuicklookGenerator`` class exported from ``grdl.discovery``.

- **Flexible channel selection** for all polarimetric decomposition
  ``to_rgb()`` methods.  Added ``channels`` parameter (``Optional[List[str]]``)
  to 19 decomposition classes for custom RGB channel mapping.  Refactored
  power-decomposition classes (``m_chi_cp``, ``m_delta_cp``, ``model_free_dp``,
  ``s_omega_cp``) to use shared ``_build_power_rgb()`` helper.  Added
  per-component rendering logic to ``Pauli``, ``Shannon``, ``Neumann``,
  ``Praks``, ``H/A/alpha`` (FP and DP), and ``ScatteringPowerDP``.  Updated
  abstract base class signatures in ``base.py`` and ``compact_pol_base.py``.
  Grayscale classes (``dop``, ``dprbi``, ``dprsi``) accept ``channels`` for
  API uniformity but always output grayscale.

### Fixed

- **``gen_quicklooks``** CLI: ``--freq-multilook`` is no longer mutually
  exclusive with ``--backend`` and ``--no-gpu``, allowing users to specify
  backend when using frequency-domain multilook (e.g.,
  ``--freq-multilook --backend torch``).
